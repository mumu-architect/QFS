...
##### 11.log管理
###### 11.1.log写入本地文件json格式行存储，每个文件128M
###### 11.2.节点重启，从本地log获取日志，重新写入内存
###### 11.3.节点重启，从本地log日志读取最新日志自增编号，同时请求shard的leader获取最新的自增id，读取主节点的数据到获取的leader之前读取的最大自增id,接下来机器提供正常服务
##### 11. 1.文件上传,分片上传[已完成]
##### 12. 1.上传文件成功后同步到从节点[未完成]
##### 13.开发对象存储oss，存储图片字段:
