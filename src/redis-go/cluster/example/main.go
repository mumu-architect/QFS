...
func main() {
	...
	flag.Parse()
	if node.ShardID == 0 || node.NodeID == 0 || node.ShardIDS == "" || node.IP == "" || node.Port == 0 || node.RpcPort == 0 || node.Peers == "" || node.NodeInfo == "" || node.RaftPort == 0 || node.RaftPeers == "" || node.RaftNodeInfo == "" || node.LogPort == 0 || node.LogPeers == "" {
		log.Fatal("必须输入--ShardID --NodeID --ShardIDS  --ip --port --RpcPort --peers  --NodeInfo --RaftPort --RaftPeers --raftNodeInfo --LogPort --LogPeers ")
	}
	...
	go func() {
		...
		//TODO:集群管理
		cl := cluster.NewCluster(node.ShardID, leaderId, node.NodeID, node.IP, node.Port, node.RpcPort, node.Peers, node.NodeInfo, nodeSlotMetas, nh, nodeMeta, nl, nf)
		...
		go func() {
			...
			for range tt.C {
				...
				fmt.Printf("111====nodeMeta.GetShardNodeLeaderID data:%v \n", shardNodeLeaderID)
				if shardNodeLeaderID == nil {
					return
				}
				nl.DynamicallyModifyLogNodeLeaderID(shardNodeLeaderID)
				nf.DynamicallyModifyFileNodeLeaderID(shardNodeLeaderID)
		...
		//		shardNodeLeaderID, err := nodeMeta.GetShardNodeLeaderID(nh, nodeMeta, node.ShardID)
		//		if err != nil {
		//			fmt.Printf("333====nodeMeta.GetShardNodeLeaderID error:%v \n", err)
		//		}
		//		fmt.Printf("333====nodeMeta.GetShardNodeLeaderID data:%v \n", shardNodeLeaderID)
		//		if shardNodeLeaderID == nil {
		//			continue
		//		}
		//		if nf.ShardId == node.ShardID {
