...
func main() {
	...
	flag.Parse()
	if node.ShardID == 0 || node.NodeID == 0 || node.ShardIDS == "" || node.IP == "" || node.Port == 0 || node.RpcPort == 0 || node.RpcNodeInfo == "" || node.Peers == "" || node.NodeInfo == "" || node.RaftPort == 0 || node.RaftPeers == "" || node.RaftNodeInfo == "" || node.LogPort == 0 || node.LogPeers == "" || node.FIlePort == 0 || node.FilePeers == "" {
		log.Fatal("必须输入--ShardID --NodeID --ShardIDS  --ip --port --RpcPort --RpcNodeInfo --peers  --NodeInfo --RaftPort  --RaftPeers --raftNodeInfo --LogPort --LogPeers  --FilePort --FilePeers")
	}
	...
	go func() {
		...
		//TODO:集群管理
		cl := cluster.NewCluster(node.ShardID, leaderId, node.NodeID, node.IP, node.Port, node.RpcPort, node.RpcNodeInfo, node.Peers, node.NodeInfo, nodeSlotMetas, nh, nodeMeta, nl, nf)
