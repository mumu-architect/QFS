...
func (nl *NodeLog) RegisterLogHandlers() {
	nl.ServeMux.HandleFunc("/syncLog", func(w http.ResponseWriter, r *http.Request) {
		...
		resp := SyncResponse{
			...
			IncrementFileMaxIdx: maxIdx,
		}
		_ = json.NewEncoder(w).Encode(resp)
	})

	nl.ServeMux.HandleFunc("/IncrementSyncLog", func(w http.ResponseWriter, r *http.Request) {
		key := r.URL.Query().Get("lastIndex")
		key2 := r.URL.Query().Get("maxIndex")
		if key == "" || key2 == "" {
			http.Error(w, "lastIndex,maxIndex不能为空", http.StatusBadRequest)
			return
		}
		//获取本地节点的log的lastIndex
		lastIndex, _ := strconv.ParseUint(key, 10, 64)
		maxIndex, _ := strconv.ParseUint(key2, 10, 64)
		// 查出所有大于lastIdx的日志
		//firstIdx, _ := LocalLog.FirstIndex()
		maxIdx := maxIndex
		// 无增量数据
