...
type Config struct {
	Server struct {
		CenterId        int64  `yaml:"center_id"`        // 数据中心id，用于雪花算法生成唯一自增id
		WorkerId        int64  `yaml:"worker_id"`        // 机器id，用于雪花算法生成唯一自增id
...
func NewDefaultConfig() *Config {
	...
	// 设置默认值
	cfg.Server.CenterId = 7
	cfg.Server.WorkerId = 262143
	cfg.Server.Address = "127.0.0.1:6379"
...
func mergeConfig(dest, src *Config) {
	// 合并Server配置
	if src.Server.CenterId > 0 {
		dest.Server.CenterId = src.Server.CenterId
	}
	if src.Server.WorkerId > 0 {
		dest.Server.WorkerId = src.Server.WorkerId
	}
